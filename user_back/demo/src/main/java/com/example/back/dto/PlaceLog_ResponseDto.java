 ...
 *
 * [이 클래스의 특별한 번역 하나]
 *  - DB의 detected_by_gps는 0/1 (Integer)인데,
 *    JSON에서는 true/false (Boolean)로 바꿔서 내보낸다.
 *  - 이유: 자바스크립트에는 0/1을 참·거짓으로 쓰는 습관이 없어서,
 *    프론트가 if (log.detectedByGps) 로 바로 쓸 수 있게 맞춰주는 것.
 *  → "경계에서 각 세계의 언어로 갈아입힌다"는 DTO의 존재 이유를 보여주는 예.
 * ============================================================
 */
@Getter
