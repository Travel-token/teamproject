...
export default function TripInfoEditDrawer({ visible, emoji, name, dateLabel, onClose, onSave, }: {
    visible: boolean;
    emoji: string;
    name: string;
    dateLabel: string;
    onClose: () => void;
    onSave: (emoji: string, name: string) => Promise<void>;
}) {
    ...
    if (!visible)
        return null;
    return (<Animated.View style={[styles.drawer, { backgroundColor: colors.bgScreen, transform: [{ translateY }] }]}>
      ...
      <View style={styles.body}>
        <View style={styles.emojiRow}>
          {TRIP_EMOJI_OPTIONS.map((e) => (<Pressable key={e} onPress={() => setSelectedEmoji(e)} style={[styles.emojiOpt, { backgroundColor: e === selectedEmoji ? colors.bgChipActive : colors.bgCard2 }]}>
              <Text style={{ fontSize: 20 }}>{e}</Text>
            </Pressable>))}
        </View>
        <FormInput value={nameInput} onChangeText={setNameInput} style={{ textAlign: 'center', fontSize: 16, fontWeight: '700', marginTop: 16, marginBottom: 12 }}/>
      ...
      <View style={styles.actions}>
        ...
        <View style={{ flex: 1 }}>
          <SubmitButton label="저장" disabled={saving} onPress={async () => {
            if (saving)
                return;
            setSaving(true);
            try {
                await onSave(selectedEmoji, nameInput.trim() || name);
                onClose();
            }
            catch {
                showToast('여행 정보 저장에 실패했어요');
            }
            finally {
                setSaving(false);
            }
