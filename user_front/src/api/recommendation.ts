...
export async function generateRecommendation(tripId: string, settlementId: number, targetUserId?: number): Promise<RecoItem> {
    const res = await api.post<ServerReco>(`/api/recommendations/trips/${tripId}`, {
        settlementId,
        targetUserId,
    });
...
export async function fetchMyRecommendations(userId?: number): Promise<RecoItem[]> {
    const res = await api.get<ServerReco[]>('/api/recommendations', {
...
export async function fetchTripRecommendations(tripId: string): Promise<RecoItem[]> {
    const res = await api.get<ServerReco[]>(`/api/recommendations/trips/${tripId}`);
    return Array.isArray(res.data) ? res.data.map(toRecoItem) : [];
}
/** PATCH /api/recommendations/{id} - 채택/수정/무시 처리 */
export async function updateRecommendationStatus(recoId: string, status: 'adopted' | 'edited' | 'dismissed', adoptedFeedPostId?: number): Promise<RecoItem> {
    const res = await api.patch<ServerReco>(`/api/recommendations/${recoId}`, {
        status,
        adoptedFeedPostId,
    });
    return toRecoItem(res.data);
...
export async function deleteRecommendation(recoId: string): Promise<void> {
    await api.delete(`/api/recommendations/${recoId}`);
}
export interface AdoptRecommendationResponse {
