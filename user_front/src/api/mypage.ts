...
export type NotificationKey = 'notifSettle' | 'notifInvite' | 'notifGps' | 'notifMarketing' | 'paySync' | 'darkMode';
...
function tripDays(t: TripHistoryApi): number {
    if (!t.startDate || !t.endDate)
        return 0;
    const diff = Math.round((new Date(t.endDate).getTime() - new Date(t.startDate).getTime()) / (1000 * 60 * 60 * 24)) + 1;
...
export async function fetchHistoryStats(): Promise<HistoryStats> {
    ...
    return {
        totalsByCurrency: statsRes.data?.totalsByCurrency,
        tripCount: trips.length,
        totalDays: trips.reduce((sum, t) => sum + tripDays(t), 0),
        placeCount: Number(statsRes.data?.placeCount ?? 0),
