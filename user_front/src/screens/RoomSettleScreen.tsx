...
// import { api } from '../api/client'; // 카카오페이 재연동 시 사용
...
export default function RoomSettleScreen({ route, navigation }: Props) {
    ...
    const reload = useCallback(async () => {
        const [balanceResult, detailResult] = await Promise.allSettled([
            fetchSettlementBalances(id), fetchSettlementDetail(id),
        ]);
        if (balanceResult.status === 'fulfilled') setBalances(balanceResult.value);
        if (detailResult.status === 'fulfilled') setSettlement(detailResult.value);
        const failure = balanceResult.status === 'rejected' ? balanceResult.reason
            : detailResult.status === 'rejected' ? detailResult.reason : null;
        if (failure) {
