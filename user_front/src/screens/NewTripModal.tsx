...
export default function NewTripModal({ visible, onClose, onCreate, }: {
    visible: boolean;
    onClose: () => void;
    onCreate: (payload: {
        name: string;
    ...
    const create = async () => {
        ...
        setSubmitting(true);
        try {
            await onCreate({ name: name.trim() || '새 여행', region: region.trim() || '미정', startDate, endDate, emoji, currency, memberNames: members.slice(1) });
            close();
        }
        catch {
            showToast('여행 저장에 실패했어요');
        }
        finally {
            setSubmitting(false);
        }
    };
    return (<BottomSheetModal visible={visible} onClose={close} title="새 여행 만들기" maxHeightPct={92}>
      <View style={styles.dots}>
        {[1, 2, 3].map((s) => (<View key={s} style={[
                styles.dot,
      ...
      {step === 1 && (<View>
          ...
          <FormRow label="아이콘 설정">
            <View style={styles.emojiGrid}>
              {EMOJIS.map((e) => (<Pressable key={e} onPress={() => setEmoji(e)} style={[
                    styles.emojiCell,
                    {
      ...
      {step === 2 && (<View>
          <FormRow label="여행 날짜">
            ...
            <View style={[styles.calWrap, { borderColor: colors.bdCard }]}>
              <Calendar onDayPress={onDayPress} markingType="period" markedDates={markedDates} theme={{
          ...
          <FormRow label="멤버 추가">
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 10 }}>
              <View style={{ flex: 1 }}>
                <FormInput value={memberInput} onChangeText={setMemberInput} placeholder="이름 입력 후 추가" onSubmitEditing={addMember}/>
              </View>
            ...
            <View style={styles.emojiGrid}>
              {members.map((m, i) => (<View key={`${m}-${i}`} style={[styles.memberChip, { backgroundColor: colors.bgCard2 }]}>
      ...
      {step === 3 && (<View>
          <FormRow label="통화 설정">
            <View style={styles.emojiGrid}>
              {CURRENCIES.map((c) => (<Pressable key={c} onPress={() => setCurrency(c)} style={[
                    styles.currencyChip,
          ...
          <View style={styles.row2}>
            ...
            <View style={{ flex: 1 }}>
              <SubmitButton label="여행 만들기" disabled={submitting} onPress={create}/>
            </View>
          </View>
        </View>)}
    </BottomSheetModal>);
