...
import { formatWon, formatMoney } from '../utils/format';
import { useTheme } from '../theme/ThemeContext';
import { ExpenseItem, PlaceItem, TransferItem } from '../types';
import { RootStackParamList } from '../navigation/types';
import { useTripDetail } from '../hooks/useTrip';
import AddEntryModal from './AddEntryModal';
import { createExpense, createTransfer, deleteExpense, deleteTransfer, fetchExpenses, fetchTransfers, updateExpense, } from '../api/expense';
...
export default function RoomExpenseScreen({ route, navigation }: Props) {
    ...
    const filteredTransferList = useMemo(() => {
        const q = searchQuery.trim();
        return q ? transferList.filter((t) => t.fromMemberName.includes(q) || t.toMemberName.includes(q)) : transferList;
    }, [transferList, searchQuery]);
    const searchResultCount = subTab === 'spend'
    ...
    const onRoomTabChange = (key: RoomTabKey) => {
        if (key === 'settle')
            navigation.replace('RoomSettle', { tripId: route.params.tripId });
        else if (key === 'map')
            navigation.replace('RoomMap', { tripId: route.params.tripId });
    };
    ...
    const removeExpense = (id: string) => confirmDelete(async () => {
        await deleteExpense(route.params.tripId, id);
        setExpenses((prev) => prev.filter((item) => item.id !== id));
    });
    const removePlace = (id: string) => confirmDelete(async () => {
        await deletePlaceLog(route.params.tripId, id);
        setPlaceList((prev) => prev.filter((item) => item.id !== id));
    });
    const removeTransfer = (id: string) => confirmDelete(async () => {
        await deleteTransfer(route.params.tripId, id);
        setTransferList((prev) => prev.filter((item) => item.id !== id));
    });
    const saveExpenseEdit = async (id: string, name: string, amount: number, payerName: string) => {
        try {
            const payer = trip.members.find((member) => member.name === payerName);
            if (!payer) {
                showToast('결제자를 찾을 수 없어요');
                return;
            }
            const updated = await updateExpense(route.params.tripId, id, {
                name,
                amount,
    ...
    return (<View style={[styles.screen, { backgroundColor: colors.bgScreen }]}>
            ...
            <View style={[styles.topBar, { paddingTop: insets.top + 12 }]}>
                {searchOpen ? (<>
                        ...
                        <View style={[styles.searchInlineBox, { backgroundColor: colors.bgInput, borderColor: colors.bdInput }]}>
                            ...
                            <TextInput value={searchQuery} onChangeText={setSearchQuery} placeholder={subTab === 'spend' ? '지출 내역 검색...' : subTab === 'place' ? '장소 검색...' : '송금 내역 검색...'} placeholderTextColor={colors.txPlaceholder} style={[styles.searchInlineInput, { color: colors.txPrimary }]} autoFocus/>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
                {subTab === 'spend' &&
            grouped.map(([date, items]) => (<View key={date} style={{ marginBottom: 8 }}>
                            <Text style={[styles.dateLbl, { color: colors.txMuted }]}>{date}</Text>
                            {items.map((e) => editingId === e.id ? (<ExpenseEditCard key={e.id} item={e} members={trip.members.map((m) => m.name)} onCancel={() => setEditingId(null)} onSave={(name, amount, payer) => saveExpenseEdit(e.id, name, amount, payer)}/>) : (<View key={e.id} style={[styles.expCard, { backgroundColor: colors.bgCard, borderColor: colors.bdCard }]}>
                                        ...
                                        <View style={{ alignItems: 'flex-end', marginRight: 8 }}>
                                            <Text style={[styles.expAmt, { color: colors.txPrimary }]}>{formatMoney(e.amount, trip.currency)}</Text>
                                            <Text style={[styles.expMy, { color: colors.txMuted }]}>내 몫 {formatMoney(e.myShare, trip.currency)}</Text>
                                        </View>
                                        <View style={{ gap: 6 }}>
                                            <Pressable onPress={() => setEditExpense(e)} style={[styles.iconSmBtn, { backgroundColor: colors.expEditBg }]}>
                ...
                {subTab === 'place' && (<View>
                        {filteredPlaceList.map((p) => (<View key={p.id} style={[styles.expCard, { backgroundColor: colors.bgCard, borderColor: colors.bdCard, marginHorizontal: 20 }]}>
                                <View style={[styles.expIcon, { backgroundColor: colors.bgCard2 }]}>
                ...
                {subTab === 'transfer' && (<View style={{ paddingHorizontal: 20 }}>
                        {filteredTransferList.map((t) => (<View key={t.id} style={[styles.transferCard, { backgroundColor: colors.bgCard, borderColor: colors.bdCard }]}>
                                <View style={[styles.expIcon, { backgroundColor: colors.bgCard2 }]}>
                                ...
                                <View style={{ flex: 1, marginLeft: 10 }}>
                                    ...
                                    <Text style={[styles.expMeta, { color: colors.txMuted }]}>
                                        {t.createdAt}{t.memo ? ` · ${t.memo}` : ''}
                                    </Text>
                                </View>
                                <Text style={[styles.expAmt, { color: colors.txPrimary }]}>{formatMoney(t.amount, trip.currency)}</Text>
                                <Pressable onPress={() => removeTransfer(t.id)} style={[styles.iconSmBtn, { backgroundColor: colors.bgDel, marginLeft: 10 }]}>
                                    <FontAwesome6 name="minus" size={11} color="#FFFFFF"/>
                                </Pressable>
                            </View>))}
                    </View>)}
            ...
            <AddEntryModal currency={trip.currency} tripId={route.params.tripId} visible={modalVisible} onClose={() => setModalVisible(false)} members={trip.members} initialTab={subTab} onSubmitExpense={async (value) => {
            try {
                const payer = trip.members.find((member) => member.id === value.payerName);
                if (!payer) {
                    showToast('결제자를 찾을 수 없어요');
                    return;
                }
                const created = await createExpense({
                    ...
                    payerMemberId: payer.id,
                    categoryCode: value.categoryCode ?? 'meal',
                    emoji: value.emoji, spentAt: value.dateLabel.length === 16 ? value.dateLabel + ':00' : value.dateLabel,
                    splitMode: value.splitMode, splits: value.participants.map(memberId => ({ memberId, amount: value.splitMode === 'manual' ? Number(value.manualAmounts[memberId] || 0) : undefined, percent: value.splitMode === 'percent' ? Number(value.percentAmounts[memberId] || 0) : undefined })),
        ...
        }} onSubmitPlace={async (value) => {
            try {
                const created = await addPlaceItem(route.params.tripId, {
                    name: value.name,
                });
                setPlaceList((prev) => [created, ...prev]);
                showToast('📍 장소가 추가됐어요');
            }
            catch {
                throw new Error('장소 추가에 실패했어요');
            }
        }} onSubmitTransfer={async (value) => {
            try {
                ...
                setTransferList((prev) => [created, ...prev]);
            ...
            {editExpense && <AddEntryModal currency={trip.currency} tripId={route.params.tripId} visible onClose={() => setEditExpense(null)} members={trip.members} initialExpense={{ name: editExpense.name, emoji: editExpense.emoji, amount: String(editExpense.amount), payerName: editExpense.payerMemberId, participants: editExpense.splits?.length ? editExpense.splits.map(s => s.memberId) : trip.members.map(m => m.id), splitMode: editExpense.splitMode ?? 'even', manualAmounts: Object.fromEntries((editExpense.splits ?? []).map(s => [s.memberId, String(s.amount ?? 0)])), percentAmounts: Object.fromEntries((editExpense.splits ?? []).map(s => [s.memberId, String(s.percent ?? 0)])), dateLabel: editExpense.spentAt ?? '', memo: editExpense.memo ?? '', categoryCode: editExpense.categoryCode }} onSubmitExpense={async (v) => { const saved = await updateExpense(route.params.tripId, editExpense.id, { name: v.name, amount: Number(v.amount), payerMemberId: v.payerName, categoryCode: v.categoryCode ?? 'meal', emoji: v.emoji, spentAt: v.dateLabel.length === 16 ? v.dateLabel + ':00' : v.dateLabel, memo: v.memo, splitMode: v.splitMode, splits: v.participants.map(memberId => ({ memberId, amount: v.splitMode === 'manual' ? Number(v.manualAmounts[memberId] || 0) : undefined, percent: v.splitMode === 'percent' ? Number(v.percentAmounts[memberId] || 0) : undefined })) }); setExpenses(prev => prev.map(e => e.id === saved.id ? saved : e)); }} onSubmitPlace={async () => { throw Error('지출 수정 화면입니다'); }} onSubmitTransfer={async () => { throw Error('지출 수정 화면입니다'); }}/>}
            <RoomMenuOverlay menuOpen={menuOpen} onCloseMenu={() => setMenuOpen(false)} emoji={trip.emoji} name={trip.name} dateLabel={trip.dateLabel} tripId={route.params.tripId} onSaveTripInfo={async (e, n) => { await saveTrip({ name: n, emoji: e }); }} onEndTrip={endTrip} onDeleteTrip={removeTrip} onDeleted={() => navigation.goBack()}/>
        </View>);
}
function ExpenseEditCard({ item, members, onSave, onCancel, }: {
    item: ExpenseItem;
    members: string[];
    ...
    return (<View style={[styles.expCard, { backgroundColor: colors.bgCard, borderColor: colors.bgChipActive, borderWidth: 1 }]}>
            <View style={{ flex: 1, gap: 6 }}>
                <FormInput value={name} onChangeText={setName} style={{ paddingVertical: 7, fontSize: 13, fontWeight: '600' }} autoFocus/>
                <View style={{ flexDirection: 'row', gap: 6 }}>
                    {members.map((m) => (<Pressable key={m} onPress={() => setPayer(m)} style={[styles.payerChip, { backgroundColor: payer === m ? colors.bgChipActive : colors.bgCard2 }]}>
                            <Text style={{ fontSize: 10, fontWeight: '600', color: payer === m ? '#FFF' : colors.txSecondary }}>{m}</Text>
                        </Pressable>))}
                </View>
            </View>
            <FormInput value={amount} onChangeText={setAmount} keyboardType="number-pad" style={{ width: 92, textAlign: 'right', fontWeight: '700', marginLeft: 8 }}/>
...
function SubTab({ label, active, changeSubTab }: {
    label: string;
    active: boolean;
    changeSubTab: () => void;
}) {
    const { colors } = useTheme();
    return (<Pressable onPress={changeSubTab} style={styles.subTabBtn}>
            <Text style={[
            styles.subTabLabel,
